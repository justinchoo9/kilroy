...
func TestSession_KimiRequestPolicy_AppliesMinMaxTokens(t *testing.T) {
	dir := t.TempDir()
	c := llm.NewClient()
	f := &fakeAdapter{
		name: "kimi",
	...
	sess, err := NewSession(c, tinyProfile{id: "kimi", mod: "kimi-k2.5", cw: 200_000}, NewLocalExecutionEnvironment(dir), SessionConfig{})
	if err != nil {
		t.Fatalf("NewSession: %v", err)
	}
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	...
	if reqs[0].MaxTokens == nil || *reqs[0].MaxTokens < 16000 {
		t.Fatalf("expected kimi max_tokens>=16000, got %#v", reqs[0].MaxTokens)
	}
}

func TestSession_OpenAIRequestPolicy_DoesNotForceKimiFloor(t *testing.T) {
	dir := t.TempDir()
	c := llm.NewClient()
	f := &fakeAdapter{
		name: "openai",
	...
	sess, err := NewSession(c, tinyProfile{id: "openai", mod: "gpt-5.2", cw: 200_000}, NewLocalExecutionEnvironment(dir), SessionConfig{})
	if err != nil {
		t.Fatalf("NewSession: %v", err)
	}
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	...
	if reqs[0].MaxTokens != nil {
		t.Fatalf("expected openai max_tokens to remain unset, got %d", *reqs[0].MaxTokens)
	}
}
